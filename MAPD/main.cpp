    ...
    if (vm.count("online")){
        ...
        if (screen >= 1)
            cout << "1"
                 ...
                 << taskAssignment->current_total_batch_service_time / taskLoader->all_batches.size() << ", "
        ...
        if (output != "")
        {
            ...
            stats << online.runtime / CLOCKS_PER_SEC << agentLoader->num_of_agents << "," << taskLoader->num_of_tasks << "," << ideal_cost << "," << total_cost << "," << ideal_delay << "," << total_delay
